...
function animate() {
    ...
    // 1. Double-smoothed visual glow interpolator for fluid liquid bloom
    // 2. Halts rotation completely into attentive calm
    // ------------------------------------------------------------------------
    const rawVoice = (currentState === 'LISTENING') 
        ? Math.max(currentMicLevel, 0.85) 
        : currentMicLevel;

    // Silky smooth visual envelope interpolator: perfectly eliminates any micro-jitter
    smoothVoiceGlow += (rawVoice - smoothVoiceGlow) * 0.10;
    const voiceGlow = smoothVoiceGlow;

    // Poly-rhythmic breathing with silky mic modulation
    ...
    // Rotation decelerates promptly to absolute zero as voice is detected
