...
const particleVertexShader = `
    ...
    void main() {
        ...
        float curAngle = angVel * time * (0.85 + energy * 0.3) * speedMul;
...
function animate() {
    ...
    // When listening to voice: instead of rotating, rotation calms down and core glows radiantly!
    const voiceGlow = currentMicLevel;
    const effectiveSpeed = currentSpeedMul * Math.max(0.10, 1.0 - voiceGlow * 0.90);
    ...
    nebulaCore.scale.set(
        (4.4 + voiceGlow * 3.0) * totalPulse * currentEnergy,
        (4.4 + voiceGlow * 3.0) * totalPulse * currentEnergy,
        1
    );
    nebulaMat.opacity = 0.50 + voiceGlow * 0.45;

    // 2. Hot incandescent inner spark flares up
    innerGlow.scale.set(
        (1.8 + voiceGlow * 1.8) * totalPulse,
        (1.8 + voiceGlow * 1.8) * totalPulse,
        1
    );
    ...
    voiceAura.scale.set(
        (5.5 + voiceGlow * 4.0) * totalPulse,
        (5.5 + voiceGlow * 4.0) * totalPulse,
        1
    );
    voiceAuraMat.opacity = voiceGlow * 0.65;

    // 4. Point light illuminates the surrounding 3D stars brilliantly
    coreLight.intensity = (1.8 + currentEnergy * 0.6) + voiceGlow * 6.0;
