...
const particleVertexShader = `
    ...
    void main() {
        ...
        // Pass voice glow intensity to fragment shader
        if (layerType == 0.0) {
            vGlowBoost = uVoiceGlow * 3.4;
        } else if (layerType == 2.0 || layerType == 3.0) {
            float innerProx = max(0.0, 1.0 - radius / 3.6);
            vGlowBoost = uVoiceGlow * innerProx * 1.8;
        } else {
            vGlowBoost = 0.0;
        }
...
function animate() {
    ...
    // ------------------------------------------------------------------------
    // VOICE LISTENING / MIC REACTIVITY:
    // When listening to voice or mic input (or in LISTENING state):
    // 1. ROTATION COMPLETELY HALTS (0.0). No opposite direction or backwards spin!
    // 2. THE CORE GLOWS RADIANTLY & FOCUSES ALL ATTENTION ON THE VOICE.
    // ------------------------------------------------------------------------
    const voiceGlow = (currentState === 'LISTENING') 
        ? Math.max(currentMicLevel, 0.85) 
        : currentMicLevel;

    // If voice/speech is detected, or in LISTENING state, rotation speed immediately ramps to ZERO
    const voiceListeningDamping = (voiceGlow > 0.03) ? Math.max(0.0, 1.0 - voiceGlow * 2.8) : 1.0;
    ...
    // 1. Nebula core scales and intensifies opacity
    nebulaCore.scale.set(
        (4.4 + voiceGlow * 3.5) * totalPulse * currentEnergy,
        (4.4 + voiceGlow * 3.5) * totalPulse * currentEnergy,
        1
    );
    nebulaMat.opacity = 0.50 + voiceGlow * 0.48;

    // 2. Hot incandescent inner spark flares up
    innerGlow.scale.set(
        (1.8 + voiceGlow * 2.2) * totalPulse,
        (1.8 + voiceGlow * 2.2) * totalPulse,
        1
    );
    ...
    voiceAura.scale.set(
        (5.5 + voiceGlow * 5.0) * totalPulse,
        (5.5 + voiceGlow * 5.0) * totalPulse,
        1
    );
    voiceAuraMat.opacity = voiceGlow * 0.75;

    // 4. Point light illuminates the surrounding 3D stars brilliantly
    coreLight.intensity = (1.8 + currentEnergy * 0.6) + voiceGlow * 7.5;
