...
const CONFIG = {
    ...
    twist: 5.2,
    armSpread: 0.62,
    vertSpread: 1.15, // 3D celestial orb volumetric depth
    rotationSpeed: 0.016,
...
const particleVertexShader = `
    ...
    void main() {
        ...
        if (layerType == 4.0 || layerType == 0.0) {
            ...
            pos.y = position.y + sin(curAngle * 2.0 + time * 0.9) * 0.08;
        }

        // Sub-bass respiration pulse
        float rScale = 1.0 + (pulse - 1.0) * (layerType == 0.0 ? 1.10 : 0.45);
        pos *= rScale;

        vec4 worldPos = modelMatrix * vec4(pos, 1.0);

        // Smooth cursor attraction & gentle swirl across entire galaxy plane
        vec3 toCursor = uMouseWorld - worldPos.xyz;
        float dist = length(toCursor.xz);
        if (dist < uMouseRadius && uMouseStrength > 0.001) {
            float normDist = dist / uMouseRadius;
            float falloff = (1.0 - normDist) * (1.0 - normDist);
            vec3 pullDir = vec3(toCursor.x, 0.0, toCursor.z) / max(0.001, dist);
            vec3 swirlDir = vec3(-pullDir.z, 0.0, pullDir.x);
            worldPos.xyz += (pullDir * 0.35 + swirlDir * 0.65) * (falloff * uMouseStrength * 0.9);
        }
...
function buildAstraGalaxy() {
    ...
    for (let i = 0; i < CONFIG.heroStarCount; i++) {
        ...
        // 3D celestial orb envelope: lenticular dome height
        const domeHeight = Math.cos((radius / CONFIG.galaxyRadius) * (Math.PI * 0.44));
        const jitter = (Math.random() - 0.5) * CONFIG.armSpread * 0.35;
        const jitterY = (Math.random() - 0.5) * CONFIG.vertSpread * (0.30 + domeHeight * 0.75);
    ...
    // A. NUCLEUS CLUSTER (1,100 sparkling pinpoint core stars, genuine spherical orb)
    for (let i = 0; i < CONFIG.nucleusCount; i++) {
        ...
        stdPos[idx * 3 + 0] = r * Math.sin(phi) * Math.cos(theta) * 1.05;
        stdPos[idx * 3 + 1] = r * Math.sin(phi) * Math.sin(theta) * 0.95; // Genuine spherical orb volume
    ...
    for (let i = 0; i < CONFIG.mediumStarCount; i++) {
        ...
        // Box-Muller normal scatter with 3D dome envelope
        const domeHeight = Math.cos((radius / CONFIG.galaxyRadius) * (Math.PI * 0.44));
        ...
        stdPos[idx * 3 + 1] = (Math.random() - 0.5) * CONFIG.vertSpread * (0.30 + domeHeight * 0.80);
    ...
    for (let i = 0; i < CONFIG.dustCount; i++) {
        ...
        stdPos[idx * 3 + 0] = rNorm * Math.cos(phi);
        stdPos[idx * 3 + 1] = (Math.random() - 0.5) * CONFIG.vertSpread * (0.28 + domeHeight * 0.75);
        stdPos[idx * 3 + 2] = rNorm * Math.sin(phi);
...
const galaxyPlane = new THREE.Plane(new THREE.Vector3(0, 1, 0), 0); // Intersect with actual galaxy plane Y=0
...
window.addEventListener('mousemove', (e) => {
    ...
    if (mouseRaycaster.ray.intersectPlane(galaxyPlane, mouseWorldPoint)) {
...
function updateMicrophone() {
    if (!isMicActive || !analyser || !audioDataArray) {
        ...
        const avg = sum / count;
        // Smooth linear response with soft floor
        targetMicLevel = Math.min(1.0, Math.max(0.0, (avg - 10.0) / 65.0));
    }
    // Asymmetric smooth envelope follower: gentle attack (0.07), long smooth decay (0.025)
    const rate = (targetMicLevel > currentMicLevel) ? 0.07 : 0.025;
...
function animate() {
    ...
    // Subtle poly-rhythmic breathing with silky smooth mic modulation
    const subBass = 1.0 + Math.sin(t * 1.2) * 0.025 * currentEnergy;
    const micPulse = 1.0 + currentMicLevel * 0.15; // Gentle 15% max respiration swell (no violent jerking)
    const totalPulse = subBass * micPulse;

    // Smooth fluid acceleration without harsh jerkiness
    const effectiveSpeed = currentSpeedMul * (1.0 + currentMicLevel * 0.25);
    ...
    nebulaCore.scale.set(4.8 * totalPulse * currentEnergy, 4.8 * totalPulse * currentEnergy, 1);
    nebulaMat.opacity = 0.50 + currentMicLevel * 0.20;
    innerGlow.scale.set(2.0 * totalPulse, 2.0 * totalPulse, 1);
    coreLight.intensity = (1.8 + currentEnergy * 0.6) * (1.0 + currentMicLevel * 0.35);
