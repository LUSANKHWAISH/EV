...
Item {
    ...
    // Characters revealed per timer tick (1 for crisp typewriter cadence)
    readonly property int _charsPerTick: 1
    // Timer interval in ms — 16ms ≈ 60 ticks/sec for smooth feel
    readonly property int _tickInterval: 16
    ...
    Column {
        ...
        // Header: Centered AI Response label + subtle Lifecycle HUD + subtle dismiss
        Item {
            id: hudHeader
            width: parent.width
            height: Math.max(20, Math.max(lifecycleHUD.implicitHeight, dismissText.implicitHeight))
            visible: root.isVisible

            // Subtle glowing centered AI Response label
            Row {
                anchors.centerIn: parent
                spacing: Theme.spacingXS
                visible: root.resultAvailable || root.isRunning
                ...
                Text {
                    text: root.isRunning ? "PROCESSING" : "AI RESPONSE"
        ...
        Flickable {
            ...
            readonly property real maxTextHeight: root.isCompactHeight
                ? Math.min(parent.parent ? parent.parent.height * 0.18 : 85, 85)
