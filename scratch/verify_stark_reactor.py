...
def run_test():
    ...
    out_dir = Path("d:/EV/scratch/stark_reactor_renders")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    def step1_idle():
        img = window.grabWindow()
        path = str(out_dir / "stark_idle.png")
        img.save(path)
        print(f"Captured IDLE: {path}")
        
        # Make a core crop natively with QImage
        crop = img.copy(600, 180, 720, 640)
        crop.save(str(out_dir / "stark_core_crop.png"))
        print("Saved core crop!")
    ...
    def step2_speaking():
        img = window.grabWindow()
        path = str(out_dir / "stark_speaking.png")
        img.save(path)
    ...
    def step3_music():
        img = window.grabWindow()
        path = str(out_dir / "stark_music.png")
        img.save(path)
