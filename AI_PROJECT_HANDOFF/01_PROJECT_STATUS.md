...
- **Cinematic Presentation & 3D Nucleus (Astra Core)**:
  - GPU-instanced Qt Quick 3D celestial sphere (`gui/qml/components/presets/EVCoreFlagshipVisual.qml` and `prototypes/cinematic_v4/qml/NucleusView.qml`).
  - Bounded camera frustum at FOV 38° ensuring unclipped, fully spherical presentation across all normal orientations and zoom levels (resolved previous 27.5° frustum box-clipping issue).
  - Independent full-display atmospheric fire particle layer (`OrbitalAura.qml` with 1,400 instances, $\le 8.0\text{ px}$ diameter, 1 draw call, isolated from nucleus drag/zoom).
