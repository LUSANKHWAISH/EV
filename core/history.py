...
# Explicit, bounded allow-lists for typed component-result attachment (012D).
# Only these scalar/metadata fields are ever persisted into task_history_events.
# Notably, VerificationResult.evidence_summary is intentionally NOT listed: it can
# embed full tool results (e.g. a TextReadResult with entire file contents).
...
class EVTaskHistoryStore:
    ...
    # -----------------------------------------------------------------
    # 012D: typed verification / risk / backup / restore attachment
    #
    # These are thin, explicitly bounded wrappers over append_event. They take an
    # explicit task_id (the result models deliberately do NOT carry task_id) and
    # emit only allow-listed scalar/metadata fields. Full evidence, file contents
    # and unrestricted model serialization are never persisted.
    # -----------------------------------------------------------------
    @classmethod
    def _bounded_payload(cls, result: Any, allowed_fields: tuple) -> dict:
        """
        Build an attachment payload from an explicit field allow-list.

        Reads attributes directly from the object (never model_dump of the whole
        model), so fields outside the allow-list can never leak into history.
        """
        payload: dict = {"type": type(result).__name__}
        for field in allowed_fields:
            if hasattr(result, field):
                payload[field] = cls._to_jsonable(getattr(result, field))
        return payload
    ...
    def attach_verification(
        ...
        *,
        ...
        """Attach a bounded VerificationResult as a VERIFICATION_RESULT event.

        The verifier's ``evidence_summary`` (which can embed full file contents via
        a TextReadResult) is intentionally excluded from the persisted payload.
        """
        if not isinstance(result, VerificationResult):
            raise TypeError("result must be a VerificationResult")
        payload = self._bounded_payload(result, _VERIFICATION_PAYLOAD_FIELDS)
    ...
    def attach_risk_assessment(
        ...
        *,
        ...
        """Attach a bounded RiskAssessmentResult as a RISK_ASSESSMENT event."""
        if not isinstance(result, RiskAssessmentResult):
            raise TypeError("result must be a RiskAssessmentResult")
        payload = self._bounded_payload(result, _RISK_PAYLOAD_FIELDS)
        return self.append_event(
            task_id,
            TaskHistoryEventType.RISK_ASSESSMENT,
            payload,
            created_at=created_at,
        )
    ...
    def attach_backup(
        ...
        *,
        ...
        """Attach a bounded BackupResult as a BACKUP_RESULT event.

        Includes a bounded ``backup_record`` (sizes/hash/flags only) when present.
        No file contents are persisted.
        """
        if not isinstance(result, BackupResult):
            raise TypeError("result must be a BackupResult")
        payload = self._bounded_payload(result, _BACKUP_PAYLOAD_FIELDS)
        if result.backup_record is not None:
            payload["backup_record"] = self._bounded_payload(
                result.backup_record, _BACKUP_RECORD_PAYLOAD_FIELDS
            )
        return self.append_event(
            task_id,
            TaskHistoryEventType.BACKUP_RESULT,
            payload,
            created_at=created_at,
        )
    ...
    def attach_restore(
        ...
        *,
        ...
        """Attach a bounded RestoreResult as a RESTORE_RESULT event."""
        if not isinstance(result, RestoreResult):
            raise TypeError("result must be a RestoreResult")
        payload = self._bounded_payload(result, _RESTORE_PAYLOAD_FIELDS)
        return self.append_event(
            task_id,
            TaskHistoryEventType.RESTORE_RESULT,
            payload,
            created_at=created_at,
        )
