...
Item {
    ...
    // Interactive mouse orbit tilt with smooth spring-back
    property real mouseTiltX: 0.0
    property real mouseTiltY: 0.0

    Behavior on mouseTiltX {
        SpringAnimation { spring: 3.0; damping: 0.4 }
    }
    ...
    View3D {
        ...
        Node {
            ...
            // ----------------------------------------------------------------
            // Cool Hero Stars (154 entries)
            Model {
                id: heroCoolStars
                source: "#Rectangle"
                scale: Qt.vector3d(0.092, 0.092, 0.092)
    ...
    MouseArea {
        ...
        onPositionChanged: function(mouse) {
            var centerX = root.width * 0.5;
            var centerY = root.height * 0.5;
            var dx = (mouse.x - centerX) / centerX;
            var dy = (mouse.y - centerY) / centerY;

            // Subtle interactive tilt range (+/- 14 degrees)
            root.mouseTiltY = dx * 14.0;
            root.mouseTiltX = -dy * 14.0;
        }
